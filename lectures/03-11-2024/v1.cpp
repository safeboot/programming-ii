...
public:
	...
	float getProsjecnaOcjena() {
		...
		for (int i = 0; i < getTrenutnaOcjena(); i++) {
			suma += _ocjene[i];
		}
		
		return suma / getTrenutnaOcjena();
	...
	void setGlumackaPostava(Glumac* glumci, int velicina) {
		...
		_trenutnoGlumaca = velicina;
		
		for (int i = 0; i < _trenutnoGlumaca; i++) {
			_glumackaPostava[i].setAll(glumci[i]);	// Prepisivanje preko starog objekta
		}
	...
	void setOcjene(int* ocjene, int velicina) {
		...
		_trenutnaOcjena = velicina;
		
		// Brisanje starih ocjena
		delete[] _ocjene;
		
		_ocjene = new int[_trenutnaOcjena];
	...
	void setAll(const char* filmID, const char* naziv, const char* zanr, int godinaIzlaska) {
	...
	bool dodajOcjenu(int ocjena) {
		...
		*/
		...
		int* temp = _ocjene;
		_ocjene = new int[_trenutnaOcjena + 1];
		
		// Kopiranje iz starog u novi niz
		for (int i = 0; i < _trenutnaOcjena; i++) {
			_ocjene[i] = temp[i];	// _glumci[i].setAll(temp[i]);
		}
		
		delete[] temp;
		temp = nullptr;
		
		_ocjene[_trenutnaOcjena] = ocjena;
		
		// _glumci[_trenutnoGlumaca].setAll(glumac);
	...
	void ispis() {
		...
		cout << "Naziv filma: " << getNaziv() << endl;
		cout << "Zanr: " << getZanr() << endl;
		cout << "Godina izlaska: " << getGodinaIzlaska() << endl;
		cout << "Trenutno glumaca: " << getTrenutnoGlumaca() << endl;
		
		cout << "====================================================" << endl;
		
		for (int i = 0; i < getTrenutnoGlumaca(); i++) {
			_glumackaPostava[i].ispis();
		}
		
		cout << "====================================================" << endl;
		
		cout << "Prosjecna ocjena filma: " << getProsjecnaOcjena() << endl;
		cout << "Ukupno ocjena: " << getTrenutnaOcjena() << endl;
...
void zadatak3() {
	...
	alPacino.setAll("2504940555551", "Alfredo", "Pacino", temp1, "Manhattan, NYC", "SAD", true);
